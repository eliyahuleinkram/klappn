 ...
 *
 * Runs autoShapeSong, which is two shapes, each ONE whole-song call on the
 * same tier (2026-08-04): the effect glides, which span the piece and need
 * the whole arc — then every turn as one set, graded across the song, each
 * told the glides crossing it. Both sets are REPLACED wholesale (the pill
 * says so before the tap; empty clears — but a whiffed call leaves what rides
 * untouched). Owner-scoped, billed like any generation. Returns the fresh
 * lists so the page can swap them in without a reload — plus `result`, what
 * the sweep actually DID, so a no-op can say so instead of miming success
 * (2026-07-31).
 */
