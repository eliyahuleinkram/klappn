...
export const ROUTE = {
  ...
  // (ROUTE.turn — one Sonnet call per seam — lived here 08-02→08-04. Its own
  // comment said "if the turns come out bland, this line is the one thing
  // that moves"; they did — three seams, three identical tom cascades, each
  // blind call reaching for the same obvious answer — and what moved was the
  // whole job: every turn is ONE call on ROUTE.shape now, beside the effects,
  // because variety and escalation only exist across the set.)
};
